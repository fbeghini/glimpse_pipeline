...
// }
	...
	// 	set file(vcf), file(tsv), file(tbi) from Channel.fromPath(params.sam_files).map{vcf -> [sam_file, vcf, vcf + ".tsv.gz", vcf + ".tsv.gz.tbi"] }
	// 	output:
	// 	file "*.vcf.gz"
	// 	file "*csi"
	// 	publishDir "${params.out}/results/bam_files", pattern: "{*vcf*}", mode: "move"
	// 	"""
	// 	bcftools mpileup --threads ${task.cpus} -f ${params.reference_genome} -I -E -a 'FORMAT/DP' -T ${vcf} ${sam_file} -Ou | bcftools call -Aim -C alleles -T ${tsv} -Oz -o ${sam_file.baseName}_${vcf.baseName}.vcf.gz
	// 	bcftools index -f ${sam_file.baseName}_${vcf.baseName}.vcf.gz
	// 	"""
	// }
...
process chunk {
	...
	chrom=`bcftools index --threads ${task.cpus} -s ${sites_vcf} | cut -f1`
 	...
 	GLIMPSE_chunk --thread ${task.cpus} --input ${sites_vcf} --region \${chrom} --window-size ${params.window_size} --buffer-size ${params.buffer_size} --output \${chrom}.chunks.txt > \${chrom}.chunks.log
...
process study_by_chrom {
	cpus 1
	input:
	set file(vcf), file(vcf_index) from Channel.fromPath(params.study_vcf).map{ vcf -> [ vcf, vcf + ".csi" ] }
...
process reference_by_chrom {
	input:
	set file(vcf), file(vcf_index) from Channel.fromPath(params.reference_vcfs).map{ vcf -> [ vcf, vcf + ".tbi" ] }
...
// study = study.combine(chunks, by: 0)
process impute_chunks {
	...
	GLIMPSE_phase --thread ${task.cpus} --input ${study_vcf} --reference ${ref_vcf} --map ${params.glimpse_maps}chr\${chrom_no_prefix}.b38.gmap.gz --input-region \${irg} --output-region \${org} --output ${chrom}.\${id}.${study_vcf.getBaseName()}.imputed.bcf --log ${chrom}.\${id}.${study_vcf.getBaseName()}.imputed.log
	"""
}
...
process ligate_chunks {
	...
	GLIMPSE_ligate --thread ${task.cpus} --input files_list.txt --output ${chrom}.${base_name}.imputed.bcf --log ${chrom}.${base_name}.ligate.log
